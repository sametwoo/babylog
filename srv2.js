...
app.get('/query',(req, res)=>{
	...
	html="<html><head><link rel='stylesheet' type='text/css' href='style.css'/></head><body><form action='/save' method='get'><div><h3>奶粉</h3><input type='number' name='milk'/><h3>母乳</h3><input type='number' name='bm'/></div><h3>大小便</h3><input type='radio' name='poops' value='11'/>撒尿了<input type='radio' name='poops' value='12'/>尿很多<input type='radio' name='poops' value='1'/>拉屎了<input type='radio' name='poops' value='2'/>屎很多<p><input type='submit' value='提交'/></p></form>"
	var historymilk='';
	var historypoops='';
    var summary='';
    var advise='';
    var ts0, ts1;
	const pMilk=pool.query("SELECT * FROM milk ORDER BY time DESC;").then(rs=>{
        ts0=rs;
        var t0="<p><table><tr><th>哺喂时间</th><th>奶量/ml</th></tr>"
        for(i=0; i<rs.rows.length; i++) {
			t=new Date(rs.rows[i].time);
			m=rs.rows[i].milk;
			t0+="<tr><td>"+t.toLocaleString('zh-CN')+"</td><td>"+m+"</td></tr>"
		}
		t0+="</table></p>"
        historymilk=t0;
	});
	const pPoops=pool.query("SELECT * FROM poops WHERE poops>0 ORDER BY time DESC;").then(rs=>{
        ts1=rs;
        ...
        for(i=0; i<rs.rows.length; i++) {
			...
			m=rs.rows[i].poops;
			s='';
			if(m==11) {s='有小便';} else if(m==12) s='小便多'; else if(m==1) {s='有大便';} else if(m==2) {s='大便多';}
			t1+="<tr><td>"+t.toLocaleString('zh-CN')+"</td><td>"+s+"</td></tr>"
		}
		t1+="</table></p>"
        historypoops=t1;
	});
    const pSummary=Promise.allSettled([pPoops,pMilk]).then(()=>{
        lastFeedTime=Date.parse(ts0.rows[0].time);
    ...
    const pAdvise=pool.query('SELECT time, milk FROM milk WHERE EXTRACT(EPOCH FROM (now() - time)) < 7*24*3600 ORDER BY time ASC;').then(rs=>{
        v0=0;
        for(i=0; i<rs.rows.length; i++) {
            v0+=rs.rows[i].milk;
        }
...
app.get('/save',(req, res)=>{
	...
	const p2=pool.query("INSERT INTO milk (time, milk) VALUES ($1,$2);",[t.toISOString(),parseInt(req.query['milk'])]);
